...
echo "public String hibernateCfg = \"/hibernate-aladdin-sc-custom.cfg.xml\";" >> StorageComponent/src/com/aladdin/sc/config/Configuration.java
...
cat build.xml |sed 's/<property name="name" value="StorageComponent"\/>/<property name="name" value="SCC"\/>/' > tmp
mv tmp build.xml

cat resources/services.xml|sed 's/<service name="StorageComponent">/<service name="SCC">/' > tmp
mv tmp resources/services.xml

cat resources/StorageComponent.wsdl|sed 's/<wsdl:service name="StorageComponent">/<wsdl:service name="SCC">/' > tmp
mv tmp resources/StorageComponent.wsdl

cat ./src/com/aladdin/raac/RaacStub.java|sed 's/http:\/\/193.174.152.114:8080\/axis2\/services\/raac\//http:\/\/127.0.0.1:8080\/axis2\/services\/raacNHNN\//' > tmp
