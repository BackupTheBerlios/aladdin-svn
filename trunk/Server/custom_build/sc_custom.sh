...
echo "public static String hibernateCfg = \"/hibernate-aladdin-sc-$1.cfg.xml\";" >> src/com/aladdin/sc/config/Configuration.java
echo "public static String forumSC = \"http://dafnis.atosorigin.es/$1/Aladdin/includes/sc.php\";" >> src/com/aladdin/sc/config/Configuration.java
...
cat build.xml |sed "s/<property name=\"name\" value=\"StorageComponent\"\/>/<property name=\"name\" value=\"SC$1\"\/>/" > tmp
mv tmp build.xml

cat resources/services.xml|sed "s/<service name=\"StorageComponent\">/<service name=\"SC$1\">/" > tmp
mv tmp resources/services.xml

cat resources/StorageComponent.wsdl|sed "s/<wsdl:service name=\"StorageComponent\">/<wsdl:service name=\"SC$1\">/" > tmp
